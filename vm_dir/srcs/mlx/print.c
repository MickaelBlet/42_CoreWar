...
/*   Updated: 2016/09/16 15:39:33 by mblet            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	vm_mlx_print(void)
{
	...
	i = 0;
	libx_image_fill(sgt_mlx()->main, 0xECECEC);
	libx_image_fill(sgt_mlx()->ram, 0x000000);
	LIBX_DRAW_SQUARE_TO_IMAGE(sgt_mlx()->main, 19, 19,
			VM_WIN_WIDTH - 718, VM_WIN_HEIGHT - 38, 0x000000);
	str = NULL;
	while (i < MEM_SIZE)
	{
		ft_asprintf(&str, "%02x", (unsigned char)sgt_corewar()->ram[i].data);
		LIBX_STRING_TO_IMAGE(sgt_mlx()->ram, sgt_mlx()->font,
	...
	ft_strdel(&str);
	LIBX_IMAGE_TO_IMAGE(sgt_mlx()->main, sgt_mlx()->ram, 20, 20,
